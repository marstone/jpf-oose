...
public class GETFIELD extends gov.nasa.jpf.symbc.bytecode.GETFIELD {
	...
	public Instruction execute(ThreadInfo ti) {
		...
		if (fi == null) {
			return ti.createAndThrowException("java.lang.NoSuchFieldError", "referencing field '" + fname + "' in " + ei);
		}

		Object attr = ei.getFieldAttr(fi);
		// check if the field is of ref type & it is symbolic (i.e. it has an
		// attribute)
		...
		if (!ti.isFirstStepInsn()) {
			...
			prevHeapCG = ti.getVM().getSystemState()
			...
			if (prevHeapCG != null) {
				...
				SymbolicInputHeap symInputHeap = ((HeapChoiceGenerator) prevHeapCG)
			...
			// here we can have 2 choice generators: thread and heappc at the
			// same time?

			frame.pop(); // Ok, now we can remove the object ref from the stack

			thisHeapCG = ti.getVM().getSystemState()
					.getLastChoiceGeneratorOfType(HeapChoiceGenerator.class);
			assert (thisHeapCG != null && thisHeapCG instanceof HeapChoiceGenerator) : "expected HeapChoiceGenerator, got: "
					+ thisHeapCG;
			currentChoice = ((HeapChoiceGenerator) thisHeapCG).getNextChoice();
		}

		PathCondition pcHeap; // this pc contains only the constraints on the
								// heap
		SymbolicInputHeap symInputHeap;

		// depending on the currentChoice, we set the current field to an object
		// that was already created
		// 0 .. numymRefs -1, or to null or to a new object of the respective
		// type, where we set all its
		// fields to be symbolic

		prevHeapCG = thisHeapCG
				.getPreviousChoiceGeneratorOfType(HeapChoiceGenerator.class);
		...
		if (prevHeapCG == null) {
			...
			symInputHeap = ((HeapChoiceGenerator) prevHeapCG)
					.getCurrentSymInputHeap();
		}
